...
from matching_app.forms.user_profile import UserFrom, UserProfileForm
...
def user_profile_update(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        user_form = UserFrom(request.POST, request.FILES, instance=request.user)
        ...
        if not user_profile_form.is_valid():
            logger.error("Failed to update user prfile", error=user_profile_form.errors)
            is_valide = False
        if is_valide:
            user_form.save()
